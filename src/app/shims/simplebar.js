...
export default class SimpleBar {
	...
	init() {
		...
		this.trackY = this.el.querySelector(
			`.${this.classNames.track}.vertical`
		)
	...
	removeListeners() {
		...
		this.scrollbarX.removeEventListener('mousedown', e =>
			this.onDrag(e, 'x')
		)
		this.scrollbarY.removeEventListener('mousedown', e =>
			this.onDrag(e, 'y')
		)
	...
	resizeScrollbar(axis = 'y') {
		...
		let handleSize = Math.max(
			~~(scrollbarRatio * (scrollbarSize - 2)) - 2,
			this.options.scrollbarMinSize
		)
		let handleOffset = ~~((scrollbarSize - 4 - handleSize) *
			scrollPourcent +
			2)
